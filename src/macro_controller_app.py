...
class MacroControllerApp:
    ...
    def _build_ui(self):
        ...
        ttk.Label(self.root, text="步骤列表（已执行=绿，当前=黄，待执行=灰）:").pack(anchor=tk.W, padx=8, pady=2)
    ...
    def _on_step_double_click(self, event):
        ...
        item = sel[0]
        try:
            step_one_based = int(item) + 1
        except (ValueError, TypeError):
            return
        self.start_step_var.set(str(step_one_based))
        self.status_var.set(f"已选择从第 {step_one_based} 步开始（双击步骤可设置）")
    ...
    def _update_step_states(self, current_index: int, total_done: int):
        ...
        self.progress_var.set(f"已执行 {total_done} 步，当前第 {current_index + 1} / {total} 步")
