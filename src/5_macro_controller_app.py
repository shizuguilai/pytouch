...
# -*- coding: utf-8 -*-
"""
宏控制桌面应用 V2 - 支持按账号勾选执行

功能：
- 加载宏 JSON（支持旧版 actions 或新版 accounts 格式）
- 新版 macro_final_2.json：可勾选要执行的账号（默认全选）
- 其余同 4_macro_controller_app：步骤列表、从指定步开始、暂停/手动控制、保存
"""
...
DEFAULT_MACRO_PATH = os.path.join(_script_dir, "macros", "macro_final_2.json")
...
def play_macro_worker(
    ...
    """
    在后台线程中执行宏。
    config 需包含 fixed_delay, random_delay, actions（可为扁平 actions 或从 accounts 合并后的列表）。
    """
...
class MacroControllerApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("宏控制 V2 - 账号勾选 / 播放 / 暂停 / 手动控制")
    ...
    def _build_ui(self):
        ...
        # 账号勾选区域（加载 macro_final_2 时在此插入复选框）
        self.account_cb_frame = ttk.LabelFrame(self.root, text="执行账号（勾选即参与播放，默认全选）", padding=6)
        self.account_cb_frame.pack(fill=tk.X, padx=8, pady=4)
        # 内容由 _load_macro_by_path 在检测到 accounts 时填充；无 accounts 时 pack_forget
        ...
        self.tree = ttk.Treeview(tree_frame, columns=("content",), show="tree headings", height=16, selectmode="browse")
    ...
    def _rebuild_actions_from_accounts(self):
        ...
        for acc_idx, acc in enumerate(self.accounts):
            ...
            if not self.account_vars.get(vid, tk.BooleanVar(value=True)).get():
                continue
            for step_idx, a in enumerate(acc["actions"]):
                self.actions.append(a)
                self._flat_to_account_map.append((acc_idx, step_idx))
        ...
        self.current_step_index = -1
        self.total_done_count = 0
        self.status_var.set(f"已选 {n} 步（来自 {sum(1 for v in self.account_vars.values() if v.get())} 个账号）")

    def _on_account_check_changed(self):
        """账号勾选变化时重建步骤列表。"""
        ...
        if "accounts" in self.config and isinstance(self.config["accounts"], list):
            ...
            for acc in self.accounts:
                ...
                cb = ttk.Checkbutton(
                    ...
                    variable=var,
                    command=self._on_account_check_changed,
                )
    ...
    def _set_dirty(self, dirty: bool):
        ...
        if dirty and self.config_path:
            self.root.title("宏控制 V2 - 账号勾选 / 播放 / 暂停 / 手动控制 *未保存*")
        else:
            self.root.title("宏控制 V2 - 账号勾选 / 播放 / 暂停 / 手动控制")
    ...
    def _open_macro(self):
        path = filedialog.askopenfilename(
            ...
            initialfile="macro_final_2.json",
    ...
    def _start_or_resume(self):
        if not self.actions:
            messagebox.showwarning("提示", "请先打开宏文件，且至少勾选一个账号")
            return
        ...
        # 使用当前勾选后的扁平 actions 作为播放列表
        run_config = {
            "fixed_delay": self.config.get("fixed_delay", 0.2),
            "random_delay": self.config.get("random_delay", 5.0),
