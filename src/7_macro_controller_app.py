...
# -*- coding: utf-8 -*-
"""
宏控制桌面应用 V3 - 账号勾选 + 制造功能勾选执行

功能：
- 加载 7_macro.json（新格式：global_actions + accounts.pre/crafting_sections/post）
- 双维度勾选：账号勾选（默认全选） + 制造功能勾选（默认全选）
- 执行时按选择组合账号的 pre_actions + 选中的 crafting_sections + post_actions
- 步骤列表（已执行=绿，当前=黄，待执行=灰）、双击编辑、保存到原文件
- 暂停 / 停止 / 手动控制
"""
...
DEFAULT_MACRO_PATH = os.path.join(_script_dir, "macros", "7_macro.json")
...
class MacroControllerApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("宏控制 V3 - 账号 + 制造功能勾选 / 播放 / 暂停 / 手动控制")
        ...
        self.global_actions = []
        self.accounts = []
        self.account_vars = {}   # vid -> BooleanVar

        # 制造功能相关
        self.crafting_options = []   # [{"id": ..., "name": ...}, ...]
        self.crafting_vars = {}      # craft_id -> BooleanVar

        # 扁平步骤溯源：("global", g_idx) | ("pre", acc_idx, step_idx)
        #               | ("craft", acc_idx, craft_id, step_idx)
        #               | ("post", acc_idx, step_idx)
    ...
    def _build_ui(self):
        ...
        self.tree = ttk.Treeview(
            tree_frame, columns=("content",), show="tree headings", height=16, selectmode="browse"
        )
        self.tree.heading("#0", text="步骤")
        self.tree.column("#0", width=80)
        self.tree.heading("content", text="内容")
        self.tree.column("content", width=480)
    ...
    def _load_macro_by_path(self, path: str):
        ...
        self.root.title(
            f"宏控制 V3 - 账号 + 制造功能勾选 | {os.path.basename(path)}"
        )
    ...
    def _rebuild_actions_from_selection(self):
        """根据账号勾选 + 制造功能勾选，重建 self.actions 与溯源表。"""
        self.actions = []
        self._flat_source_map = []
        ...
        n_crafts = len(selected_craft_ids)
        selected_craft_names = [
            opt["name"] for opt in self.crafting_options
            if opt["id"] in selected_craft_ids
        ]
        craft_str = "、".join(selected_craft_names) if selected_craft_names else "（无）"
        self.current_step_display_var.set(f"— （共 {n} 步）")
        self.status_var.set(
            f"已选 {n} 步 | {n_accs} 个账号 | 制造功能: {craft_str}"
        )
    ...
    def _sync_edit_to_config(self, flat_index: int, new_action: dict):
        ...
        try:
            if src[0] == "global":
                _, g_idx = src
                self.config["global_actions"][g_idx] = new_action

            elif src[0] == "pre":
                _, acc_idx, step_idx = src
                self.config["accounts"][acc_idx]["pre_actions"][step_idx] = new_action
                ...
                _, acc_idx, step_idx = src
                self.config["accounts"][acc_idx]["post_actions"][step_idx] = new_action
    ...
    def _update_step_states(self, current_index: int, total_done: int, from_callback: bool = True):
        ...
        if from_callback:
            next_1based = current_index + 2 if (current_index + 1 < total) else current_index + 1
        else:
            next_1based = current_index + 1
    ...
    def _start_or_resume(self):
        if not self.actions:
            messagebox.showwarning("提示", "请先打开宏文件，且至少勾选一个账号和一个制造功能")
            return
        ...
        def run():
            play_macro_worker(
                ...
                self.pause_event, self.stop_event, self._step_callback,
            )
